...
function getClient(): NwcClient {
    global $nwcUri;
    if (!$nwcUri) throw new \RuntimeException('NWC_URI not configured in .env');
    $client = new NwcClient(new NwcUri($nwcUri));
...
if (isset($_GET['get_balance'])) {
    ...
    try {
        ...
        if ($bal->isSuccess()) {
            echo json_encode(['balance' => $bal->getBalanceInSats()]);
        } else {
...
if (isset($_GET['check_invoice'])) {
    ...
    try {
        ...
        $response = $client->lookupInvoice($paymentHash);
        if (!$response->isSuccess()) {
            echo json_encode(['paid' => false, 'error' => $response->getErrorMessage()]);
        } else {
            $settled = $response->getSettledAt();
            echo json_encode([
                'paid'    => $settled !== null && $settled > 0,
                'amount'  => $response->getAmount(),
                'preimage' => $response->getPreimage(),
            ]);
        }
    } catch (\Throwable $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'receive') {
            ...
            $description = $_POST['description'] ?? 'NWC web demo';
            if ($amountSats <= 0) throw new \RuntimeException('Amount must be positive');

            $response = $client->makeInvoice($amountSats * 1000, $description);
            if (!$response->isSuccess()) {
                throw new \RuntimeException('make_invoice failed: ' . $response->getErrorMessage());
            }
            $result = [
            ...
            $response = $client->payInvoice($invoice);
            if (!$response->isSuccess()) {
                throw new \RuntimeException('pay_invoice failed: ' . $response->getErrorMessage());
            }
            $result = [
                'type'     => 'payment_sent',
                'preimage' => $response->getPreimage(),
                'fees'     => $response->getFeesPaid() ?? 0,
            ];
        }
    } catch (\Throwable $e) {
        $error = $e->getMessage();
...
try {
    $client = getClient();
    $bal = $client->getBalance();
    if ($bal->isSuccess()) $balance = $bal->getBalanceInSats();
} catch (\Throwable $e) {
    // ignore
}
...
<body>
    <div class="header">
        <h1>NWC Lightning Demo</h1>
        <p class="subtitle">Nostr Wallet Connect (NIP-47) over PHP</p>
    </div>
    ...
    <div class="balance-bar">
        ...
        <?php else: ?>
            <span class="na" id="balance-value">NWC_URI not configured</span>
        <?php endif; ?>
    </div>
    ...
    <div class="grid">
        <div class="card">
            ...
            <form method="POST">
                ...
                <input type="text" name="description" id="description" value="NWC web demo" placeholder="Payment description">
